...
import { setLoadingButton, uploadImageProduct } from "../../redux/product";
...
const DropzonePic = ({ myFiles, setMyFiles }) => {
  ...
  const files = myFiles.map((file) => (
    <div key={file.path || file.id}>
      <div style={thumb} key={file.path}>
        <div style={thumbInner}>
          <img
            src={file.picture || file.preview}
  ...
  useEffect(() => {
    ...
    myFiles.map((file) => {
      if (file.picture) {
        ...
        dispatch(uploadImageProduct(formData)).then((res) => {
          // URL.revokeObjectURL(res.payload.data[0].path);
          Object.assign(file, {
            preview: res.payload.data[0].path,
          });
        });
      }
    });
